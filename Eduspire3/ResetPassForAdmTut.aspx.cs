...
namespace Eduspire3
{
    public partial class ResetPassForAdmTut : System.Web.UI.Page
    {
        ...
        protected void btnReset_Click(object sender, EventArgs e)
        {
            if (txtNewPass.Text != "" && txtConfPass.Text != "" && txtNewPass.Text == txtConfPass.Text)
            {
                using (SqlConnection con = new SqlConnection(CS))
                {
                    SqlCommand cmd = new SqlCommand("update [Admin_Table7] set adm_Password='" + txtNewPass.Text + "' where adm_ID='" + Uid + "'", con);
                    con.Open();
                    cmd.ExecuteNonQuery();
