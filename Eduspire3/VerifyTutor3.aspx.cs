...
namespace Eduspire3
{
    public partial class VerifyTutor3 : System.Web.UI.Page
    {
        ...
        private void AutoID()
        {
            ...
            connection.Open();
            SqlCommand cmd = new SqlCommand("Select top 1 * from ActivateTut_Table2 order by ID Desc", connection);
            //SqlCommand cmd = new SqlCommand("Select count(ID)from[ActivateTut_Table2]", connection);
        ...
        private Boolean CheckEmailInCandidate()
        {
            Boolean emailavailable = false;
            SqlConnection con = new SqlConnection(CS);
        ...
        private Boolean CheckEmailInActivate()
        {
            Boolean emailavailable = false;
            SqlConnection con = new SqlConnection(CS);
            String query = "Select * from ActivateTut_Table2 where Email='" + txtEmailID.Text + "'";
            SqlCommand com = new SqlCommand(query, con);
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = com;
            DataSet ds = new DataSet();
        ...
        //objects to transfer from one page to another
        

        //method for code generation and trigger
        ...
        {
            String mycon = ConfigurationManager.ConnectionStrings["dbcon"].ConnectionString;
            String updatedata = "update ActivateTut_Table2 set activationcode = 0 , status = 'Verified' where Email='" + txtEmailID.Text + "'";
        ...
        protected void btnVerifyEmail_Click1(object sender, EventArgs e)
        {
            ...
            else if(CheckEmailInCandidate()==true)
            {
                Response.Write("<script>alert('An account with Email " + txtEmailID.Text + " has already applied.')</script>");
            }
            ...
            else
            {
                ...
                if (CheckEmailInActivate() == true)
                {
                    ...
                    String query = "update ActivateTut_Table2 set activationcode='" + activationcode + "', status = 'Unverified' where Email='" + txtEmailID.Text + "'";
                    String mycon = ConfigurationManager.ConnectionStrings["dbcon"].ConnectionString;
                    SqlConnection con = new SqlConnection(mycon);
                    con.Open();
                    SqlCommand cmd = new SqlCommand();
                ...
                else
                {
                    ...
                    Random random = new Random();
                    activationcode = random.Next(1001, 9999).ToString();
                    String query = "insert into ActivateTut_Table2(ID,Email,Status,activationcode) values('" + lblID.Text + "','" + txtEmailID.Text + "','Unverified','" + activationcode + "')";
                    String mycon = ConfigurationManager.ConnectionStrings["dbcon"].ConnectionString;
                    SqlConnection con = new SqlConnection(mycon);
                    con.Open();
                    SqlCommand cmd = new SqlCommand();
        ...
        protected void btnVerify_Click(object sender, EventArgs e)
        {
            ...
            String myquery = "Select * from ActivateTut_Table2 where Email='" + txtEmailID.Text + "'";
            SqlConnection con = new SqlConnection(mycon);
            SqlCommand cmd = new SqlCommand();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (activationcode == txtActivationCode.Text)
                {
                    ...
                    Response.Redirect("Application.aspx?email=" + txtEmailID.Text);
